...
def test_database_to_intermediary_with_multiple_schemas(postgresql_db):
...
def test_table_names_in_relationships(postgresql_db):
    tables, relationships = database_to_intermediary(postgresql_db)
...
def test_table_names_in_relationships_with_schema(postgresql_db):
    schema_name = "test"
    matcher = re.compile(rf"{schema_name}\.[\S+]", re.I)
    tables, relationships = database_to_intermediary(postgresql_db, schema=schema_name)
    table_names = [t.name for t in tables]
